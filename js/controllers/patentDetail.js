...
soiControllers.controller('patentDetailController', ['util', '$scope', '$rootScope', '$state', '$stateParams','panelFieldsService','modelService', 'remoteDataService',
  function (util, $scope, $rootScope, $state, $stateParams, panelFieldsService, modelService, remoteDataService) {
    ...
    remoteDataService.fetchRecordByProp('VPatent', '@rid', $scope.recordItemId, function(err, data) {
      if(util.defined(data,"length") && data.length > 0) {
        $scope.objData = data[0];
        var fnd = util.findWhereProp(modelService.models, 'objectType', 'VPatent');
        if(util.defined(fnd)) {
          $scope.model = fnd;

          remoteDataService.getRecordDetails('VPatent', $scope.recordItemId, function(err, data) {
    ...
    $scope.update = function() {
      util.navigate('panelItem', {panelName : 'vPatentList', recordItemId: $scope.recordItemId, mode: 'viewDetails' })
    }
